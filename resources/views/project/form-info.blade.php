<fieldset class="mb-3">
    ...
    @php
    if ('patch' === $_method) {
        $parents = Playground\Matrix\Models\Project::where('id', '!=', $data->id)->get();
    } else {
        $parents = Playground\Matrix\Models\Project::isNotClosed()->isActive()->get();
    }
    @endphp
    @if ($parents->isEmpty())
    <input type="hidden" name="parent_id" value="" />
    @else
    <x-playground::forms.column-select column="parent_id" key="label" label="Project" :records="$parents->toArray()"/>
    @endif
