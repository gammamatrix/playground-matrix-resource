...
<div class="card">
    ...
    @php $matrix = $data->matrix()->first() @endphp
    ...
    <div class="card-body">
        <form method="POST" action="{{ $routePatch }}" novalidate class="needs-validation">
            @csrf
            @method('patch')
            <div class="btn-toolbar mb-3" role="toolbar" aria-label="{{ __('Matrix Form') }}">
                <div class="input-group w-100">
                    <select class="form-select" aria-label="{{ __('set the matrix') }}" name="matrix_id" required>
                        <option selected value="">{{ __('set the matrix') }}</option>
                        @foreach ($matrices as $matrix)
                        <option value="{{$matrix->id}}">{{$matrix->title ?: $matrix->label }}</option>
                        @endforeach
                    </select>
