<div class="card">
    ...
    @php $ticket = $data->duplicate()->first() @endphp
    ...
    @php $duplicates = Playground\Matrix\Models\Ticket::all() @endphp
        ...
        <form method="POST" action="{{ $routePatch }}" novalidate class="needs-validation">
            ...
            <div class="btn-toolbar mb-3" role="toolbar" aria-label="{{ __('Ticket Form') }}">
                <div class="input-group w-100">
                    <select class="form-select" aria-label="{{ __('set the duplicate') }}" name="duplicate_id" required>
                        <option selected value="">{{ __('set the ticket') }}</option>
                        @foreach ($duplicates as $duplicate)
                        <option value="{{$duplicate->id}}">{{$duplicate->title ?: $duplicate->label }}</option>
                        @endforeach
                    </select>
