<fieldset class="mb-3">
    ...
    <x-playground::forms.column column="release_type" label="Release Type" :rules="['maxlength' => 255]" />



    @php
    if ('patch' === $_method) {
        $parents = Playground\Matrix\Models\Release::where('id', '!=', $data->id)->get();
    } else {
        $parents = Playground\Matrix\Models\Release::isNotClosed()->isActive()->get();
    }
    @endphp
    @if ($parents->isEmpty())
    <input type="hidden" name="parent_id" value="" />
    @else
    <x-playground::forms.column-select column="parent_id" key="label" label="Release" :records="$parents->toArray()"/>
    @endif
